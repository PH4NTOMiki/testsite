...
hexo.extend.filter.register('before_generate', function () {
  ...
  hexo.log.info(hidden_posts.length+' posts are marked as hidden');
});
...
hexo.extend.filter.register('after_init', function () {
  ...
  hexo.extend.generator.register('post', async function (locals) {
	...
	//hexo.log.info(JSON.stringify(generated_hidden[0]));
  ...
  config.public_generators.filter(
    ...
    hexo.log.debug('Expose hidden posts to generator: '+name);
...
// to recognize hidden posts, and manipulate whatever we want.
  hexo.extend.filter.register('after_render:html', str => {
	if (str.includes('<!-- searchpage -->') || (config.noindex && str.includes(config.html_flag))) {
