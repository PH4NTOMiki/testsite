(function($){
  ...
  <% } %>
  <% if (c.ajaxloader){ %>
    if(history.pushState){cache={},prefetched=[],cacheKey=void 0;var $main=$("#main"),$container=$("#container"),tempCache={html:$main.html(),title:document.title,url:location.href.replace(/index\.html(?!.*index\.html)/i,"")};function renderHTML(e,t){$container.removeClass("mobile-nav-on"),t||history.pushState(e,e.title,e.url),document.title=e.title,$main.html(e.html),t||scrollTo(0,0)}function ajaxLoad(e,t,n){t&&cache[e]&&renderHTML(cache[e]),!n&&cache[e]||$.get(e,function(n){n=String(n);var o=document.createElement("textarea");o.innerHTML=n.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var a=o.value,c=n.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[e]={html:c,url:e,title:a},t&&renderHTML(cache[e])},"text")}function prefetchLinks(e){$("a.main-nav-link, a.article-date").each(function(t,n){ajaxLoad(n.href,!1,!!e)}),e||$.ajax("/json-feed.json",{method:"HEAD",success:function(e,t,n){cacheKey=n.getResponseHeader("content-length"),cacheKey||(window.useGet=!0)}})}function checkForUpdates(){$.ajax("/json-feed.json",{method:window.useGet?"GET":"HEAD",success:function(e,t,n){var o=window.useGet?e.length:n.getResponseHeader("content-length");cacheKey&&cacheKey!=o&&prefetchLinks(!0),cacheKey=o}})}cache[tempCache.url]=tempCache,history.replaceState(tempCache,tempCache.title,tempCache.url),$(document).on("click","a",function(e){var t=e.target;return t.href==location.href?(e.preventDefault(),!1):!(!e.ctrlKey&&t.href&&t.origin==location.origin&&["http:","https:"].indexOf(t.protocol)>-1&&["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(t.href.toLowerCase().split(".").pop())<0)||"http:"==t.protocol&&"https:"==location.protocol||t.search||t.hash||"#"==t.href.slice(-1)?void 0:(e.preventDefault(),ajaxLoad(t.href,!0),!1)}).on("mouseover touchstart","a",function(e){var t=e.target;if(t.href&&t.origin==location.origin&&["http:","https:"].indexOf(t.protocol)>-1&&("http:"!=t.protocol||"https:"!=location.protocol)&&!t.search&&!t.hash&&"#"!=t.href.slice(-1))if(["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(t.href.toLowerCase().split(".").pop())<0)ajaxLoad(t.href);else if(prefetched.indexOf(t.href)<0){prefetched.push(t.href);var n=document.createElement("link");n.rel="prefetch",n.href=t.href,document.head.appendChild(n)}}),window.onpopstate=function(e){e.state&&renderHTML(e.state,!0)},prefetchLinks();var updatesInt=setInterval(checkForUpdates,3e4)}else location.href!=location.href.replace(/index\.html(?!.*index\.html)/i,"")&&(location.href=location.href.replace(/index\.html(?!.*index\.html)/i,""));
  <% } %>
})(jQuery);
