(function($){
  ...
  <% if (c.ajaxloader){ %>
    if(history.pushState){cache=(window.localStorage||(window.localStorage={}))&&localStorage.cache?JSON.parse(localStorage.cache):{},prefetched=[];var $main=$("#main"),$container=$("#container"),tempCache={html:$main.html(),title:document.title,url:location.href.replace(/index\.html(?!.*index\.html)/i,"")};function renderHTML(e,t){$container.removeClass("mobile-nav-on"),t||history.pushState(e,e.title,e.url),document.title=e.title,$main.html(e.html),t||scrollTo(0,0)}function ajaxLoad(e,t,c){t&&cache[e]&&renderHTML(cache[e]),!c&&cache[e]||$.get(e,function(c){c=String(c);var a=document.createElement("textarea");a.innerHTML=c.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var o=a.value,n=c.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[e]={html:n,url:e,title:o},t&&renderHTML(cache[e]),localStorage.cache=JSON.stringify(cache)},"text")}function prefetchLinks(e){$("a.main-nav-link, a.article-date").each(function(t,c){ajaxLoad(c.href,!1,!!e)}),e||$.ajax("/json-feed.json",{method:"HEAD",success:function(e,t,c){tempcacheKey=c.getResponseHeader("content-length"),tempcacheKey?cache.key&&tempcacheKey!=cache.key&&(prefetchLinks(!0),cache.key=tempcacheKey,localStorage.cache=JSON.stringify(cache)):window.useGet=!0}})}function checkForUpdates(){$.ajax("/json-feed.json",{method:window.useGet?"GET":"HEAD",success:function(e,t,c){var a=window.useGet?e.length:c.getResponseHeader("content-length");cache.key&&a!=cache.key&&prefetchLinks(!0),a!=cache.key&&(cache.key=a,localStorage.cache=JSON.stringify(cache))}})}cache[tempCache.url]=tempCache,history.replaceState(tempCache,tempCache.title,tempCache.url),localStorage.cache=JSON.stringify(cache),$(document).on("click","a",function(e){var t=e.currentTarget;return t.href==location.href?(e.preventDefault(),!1):!(!e.ctrlKey&&t.href&&t.origin==location.origin&&["http:","https:"].indexOf(t.protocol)>-1&&["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(t.href.toLowerCase().split(".").pop())<0)||"http:"==t.protocol&&"https:"==location.protocol||t.search||t.hash||"#"==t.href.slice(-1)?void 0:(e.preventDefault(),ajaxLoad(t.href,!0),!1)}).on("mouseover touchstart","a",function(e){var t=e.currentTarget;if(t.href&&t.origin==location.origin&&["http:","https:"].indexOf(t.protocol)>-1&&("http:"!=t.protocol||"https:"!=location.protocol)&&!t.search&&!t.hash&&"#"!=t.href.slice(-1))if(["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(t.href.toLowerCase().split(".").pop())<0)ajaxLoad(t.href);else if(prefetched.indexOf(t.href)<0){prefetched.push(t.href);var c=document.createElement("link");c.rel="prefetch",c.href=t.href,document.head.appendChild(c)}}),window.onpopstate=function(e){e.state&&renderHTML(e.state,!0)},prefetchLinks();var updatesInt=setInterval(checkForUpdates,3e4)}else location.href!=location.href.replace(/index\.html(?!.*index\.html)/i,"")&&(location.href=location.href.replace(/index\.html(?!.*index\.html)/i,""));
  <% } %>
})(jQuery);
