(function($){
  ...
  <% } %>
  <% if (c.ajaxloader){ %>
    if(history.pushState){cache={};var tempCache={html:$("#main").html(),title:document.title,url:location.href};cache[tempCache.url]=tempCache,history.replaceState(tempCache,tempCache.title,tempCache.url),$(document).on("click",'a[class~="article-"]',function(t){t.preventDefault(),$("#container").removeClass("mobile-nav-on");var e=$(t.target).closest("article"),r=e.find(".article-title").text()+" | "+$('[property="og:site_name"]').attr("content");currCache={html:e[0].outerHTML,title:r,url:t.target.href},cache[currCache.url]=currCache,history.pushState(currCache,r,t.target.href),document.title=r,$("#main > *").not(e).remove(),scrollTo(0,0)}).on("click",'a:not([class~="article-"])',function(t){var e=t.target;if(e.href==location.href)return t.preventDefault(),!1;if(e.href&&e.origin==location.origin&&["http:","https:"].indexOf(e.protocol)>-1&&["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(e.href.toLowerCase().split(".").pop())<0&&("http:"!=e.protocol||"https:"!=location.protocol)&&!e.search&&!e.hash&&"#"!=e.href.slice(-1)){t.preventDefault(),$("#container").removeClass("mobile-nav-on");var r=e.href;cache[r]?(currCache=cache[r],history.pushState(currCache,currCache.title,currCache.url),document.title=currCache.title,$("#main").html(currCache.html),scrollTo(0,0)):$.get(r,function(t){t=String(t);var e=document.createElement("textarea");e.innerHTML=t.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var c=e.value,a=t.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");currCache={html:a,url:r,title:c},cache[r]=currCache,history.pushState(currCache,currCache.title,currCache.url),document.title=currCache.title,$("#main").html(currCache.html),scrollTo(0,0)},"text")}}).on("mouseover touchstart",'a:not([class~="article-"])',function(t){console.log("triggered third event listener: ",t);var e=t.target;if(!cache[e.href]&&e.href&&e.origin==location.origin&&["http:","https:"].indexOf(e.protocol)>-1&&["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(e.href.toLowerCase().split(".").pop())<0&&("http:"!=e.protocol||"https:"!=location.protocol)&&!e.search&&!e.hash&&"#"!=e.href.slice(-1)){var r=e.href;$.get(r,function(t){t=String(t);var e=document.createElement("textarea");e.innerHTML=t.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var c=e.value,a=t.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[r]={html:a,url:r,title:c}},"text")}}),onpopstate=function(t){t.state&&($("#container").removeClass("mobile-nav-on"),document.title=t.state.title,$("#main").html(t.state.html))},$("a.main-nav-link").each(function(t,e){var r=e.href;cache[r]||$.get(r,function(t){t=String(t);var e=document.createElement("textarea");e.innerHTML=t.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var c=e.value,a=t.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[r]={html:a,url:r,title:c}},"text")})}
  <% } %>
})(jQuery);
