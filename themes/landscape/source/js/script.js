(function($){
  ...
  <% } %>
  <% if (c.ajaxloader){ %>
    if(history.pushState){cache={};var tempCache={html:$("#main").html(),title:document.title,url:location.href};cache[tempCache.url]=tempCache,history.replaceState(tempCache,tempCache.title,tempCache.url),$(document).on("click",'a[class~="article-"]',function(e){console.log("triggered first event listener: ",e),e.preventDefault();var t=$(e.target).closest("article"),r=t.find(".article-title").text()+" - "+$('[property="og:site_name"]').attr("content");currCache={html:t[0].outerHTML,title:r,url:e.target.href},cache[currCache.url]=currCache,history.pushState(currCache,r,e.target.href),document.title=r,$("#main > *").not(t).remove(),scrollTo(0,0)}).on("click",'a:not([class~="article-"])',function(e){console.log("triggered second event listener: ",e);var t=e.target;if(t.href==location.href)return e.preventDefault(),!1;if(!(t.href&&t.origin==location.origin&&["http:","https:"].indexOf(t.protocol)>-1&&[".jpg",".jpeg",".png",".gif",".pdf",".xml",".json"].indexOf(t.href.toLowerCase().slice(-6))<0)||"http:"==t.protocol&&"https:"==location.protocol||t.search||t.hash||"#"==t.href.slice(-1))return!0;e.preventDefault();var r=t.href;cache[r]?(currCache=cache[r],history.pushState(currCache,currCache.title,currCache.url),document.title=currCache.title,$("#main").html(currCache.html),scrollTo(0,0)):$.get(r,function(e){var t=document.createElement("textarea");t.innerHTML=e.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var c=t.value,a=e.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");currCache={html:a,url:r,title:c},cache[r]=currCache,history.pushState(currCache,currCache.title,currCache.url),document.title=currCache.title,$("#main").html(currCache.html),scrollTo(0,0)})}).on("mouseover touchstart",'a:not([class~="article-"])',function(e){console.log("triggered third event listener: ",e);var t=e.target;if(!cache[t.href]&&t.href&&t.origin==location.origin&&["http:","https:"].indexOf(t.protocol)>-1&&[".jpg",".jpeg",".png",".gif",".pdf",".xml",".json"].indexOf(t.href.toLowerCase().slice(-6))<0&&("http:"!=t.protocol||"https:"!=location.protocol)&&!t.search&&!t.hash&&"#"!=t.href.slice(-1)){var r=t.href;$.get(r,function(e){var t=document.createElement("textarea");t.innerHTML=e.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var c=t.value,a=e.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[r]={html:a,url:r,title:c}})}}),onpopstate=function(e){$("#main").html(e.state.html)},$("a.main-nav-link").each(function(e,t){var r=t.href;cache[r]||$.get(r,function(e){var t=document.createElement("textarea");t.innerHTML=e.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var c=t.value,a=e.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[r]={html:a,url:r,title:c}})})}
  <% } %>
})(jQuery);
