(function($){
  ...
  <% } %>
  <% if (c.ajaxloader){ %>
    if(history.pushState){cache={};var $main=$("#main"),$container=$("#container"),tempCache={html:$main.html(),title:document.title,url:location.href};function renderHTML(t,e){$container.removeClass("mobile-nav-on"),e||history.pushState(t,t.title,t.url),document.title=t.title,$main.html(t.html),e||scrollTo(0,0)}function ajaxLoad(t,e){e&&cache[t]&&renderHTML(cache[t]),cache[t]||$.get(t,function(a){a=String(a);var o=document.createElement("textarea");o.innerHTML=a.replace(/.*?<title>(.*?)<\/title>.*/i,"$1");var n=o.value,r=a.replace(/.*?<section id="main">(.*?)<\/section>.*/i,"$1");cache[t]={html:r,url:t,title:n},e&&renderHTML(cache[t])},"text")}cache[tempCache.url]=tempCache,history.replaceState(tempCache,tempCache.title,tempCache.url),$(document).on("click","a",function(t){var e=t.target;if(e.href==location.href)return t.preventDefault(),!1;!(e.href&&e.origin==location.origin&&["http:","https:"].indexOf(e.protocol)>-1&&["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(e.href.toLowerCase().split(".").pop())<0)||"http:"==e.protocol&&"https:"==location.protocol||e.search||e.hash||"#"==e.href.slice(-1)||(t.preventDefault(),ajaxLoad(e.href,!0))}).on("mouseover touchstart","a",function(t){var e=t.target;!(!cache[e.href]&&e.href&&e.origin==location.origin&&["http:","https:"].indexOf(e.protocol)>-1&&["jpg","jpeg","png","gif","pdf","xml","json"].indexOf(e.href.toLowerCase().split(".").pop())<0)||"http:"==e.protocol&&"https:"==location.protocol||e.search||e.hash||"#"==e.href.slice(-1)||ajaxLoad(e.href)}),window.onpopstate=function(t){t.state&&renderHTML(t.state,!0)},$("a.main-nav-link, a.article-date").each(function(t,e){ajaxLoad(e.href)})}
  <% } %>
})(jQuery);
